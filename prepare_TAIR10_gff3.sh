...
grep '^AT' ${AIP_HOME}/${TAIR_DATA}/${TAIR10_RELEASE}/$TAIR10_func_desc | cut -f1,3 | perl -lane 'BEGIN { %data = (); } chomp; @line = split /\t/; $line[0] =~ /(\S+)\.\d+/; $data{$1} = $line[-1]; print; END { for $locus(keys %data) { print join "\t", $locus, $data{$locus}; } }' | sort -k1,1 > Note.tsv
grep '^AT' ${AIP_HOME}/${TAIR_DATA}/${TAIR10_RELEASE}/$TAIR10_func_desc | cut -f1,4 > Curator_summary.tsv
grep '^AT' ${AIP_HOME}/${TAIR_DATA}/${TAIR10_RELEASE}/$TAIR10_func_desc | cut -f1,5 > Computational_description.tsv
